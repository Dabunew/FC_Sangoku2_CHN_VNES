...
//	DEBUGOUT( "CMainFrame::OnCreate\n" );
	...
	m_bMenu = TRUE;		// ÉÅÉjÉÖÅ[ï\é¶íÜÉtÉâÉO

	// ÉÅÉjÉÖÅ[Ç…ÉAÉNÉZÉâÉåÅ[É^ÉLÅ[ñºÇÃí«â¡
	OnRebuildMenu();

	// ÉEÉCÉìÉhÉEà íuÇÃê›íË
	RECT	rc = Config.general.rcWindowPos;
	if( !((rc.right-rc.left) <= 0 || (rc.bottom-rc.top) <= 0) ) {
		if( (m_bZoomed = Config.general.bWindowZoom) ) {
			CApp::SetCmdShow( SW_SHOWMAXIMIZED );
		}
		// ï€ë∂Ç≥ÇÍÇƒÇ¢ÇΩÉEÉCÉìÉhÉEÉTÉCÉYÇ…Ç∑ÇÈ
		m_WindowRect = rc;
		::SetWindowPos( m_hWnd, HWND_NOTOPMOST, rc.left, rc.top,
				rc.right-rc.left, rc.bottom-rc.top, 0 );
	} else {
		m_bZoomed = FALSE;
		// ÉfÉtÉHÉãÉgÉTÉCÉYÇí≤êÆÇ∑ÇÈ
		OnSetWindowSize();
	}

	// ÉEÉCÉìÉhÉEÉXÉ^ÉCÉãÇÃï€ë∂
...
//	DEBUGOUT( "CMainFrame::OnDisplayChange\n" );
	...
	// ÉtÉãÉXÉNÉäÅ[ÉìÇ»ÇÁÇŒé©ï™Ç≈É`ÉFÉìÉWíÜÇ‡ÇµÇ≠ÇÕALT+TABêÿÇËë÷Ç¶íÜ
	if( !Config.general.bScreenMode ) {
		// äÎåØÇ»ÇÃÇ≈ÉXÉåÉbÉhÇÉ|Å[ÉY
		Emu.Pause();
...
//	DEBUGOUT( "CMainFrame::OnMenuChar\n" );
	// ÉLÉìÉLÉìÇ§ÇÈÇ≥Ç¢Ç©ÇÁï¬Ç∂ÇÈÅcÇ∆âRÇÇ¬Ç≠
...
{
	// ç≈ëÂâªï\é¶ìô
	// WM_SYSCOMMANDÇ≈Ç‡ó«Ç¢ÇÃÇæÇ™ÅCÉLÉÉÉvÉVÉáÉìÇÉ_ÉuÉãÉNÉäÉbÉNÇµÇΩÇ∆Ç´Ç…óàÇ»Ç¢ÇÃÇ≈...
	switch( wParam ) {
		case	SIZE_MAXIMIZED:
			m_bZoomed = TRUE;
			{
			...
			m_WindowRect = wpl.rcNormalPosition;	// å≥ÇÃà íuÇÉRÉsÅ[
			}
			break;
		case	SIZE_RESTORED:
			m_bZoomed = FALSE;
...
{
	// ÉGÉ~ÉÖÉåÅ[ÉVÉáÉìíÜÇÃÇ›
	if( Emu.IsRunning() ) {
		// ÉXÉNÉäÅ[ÉìÉZÅ[ÉoÅ[ÇÃãNìÆÇÃó}é~
		if( wParam == SC_SCREENSAVE ) {
//			DEBUGOUT( "CMainFrame::OnSysCommand  SC_SCREENSAVE\n" );
			hResult = 1L;
			return	TRUE;
		}
		// ÉfÉBÉXÉvÉåÉCìdåπÉIÉtÇÃó}é~
...
//	DEBUGOUT( "CMainFrame::OnTimer bCursor=%s bForcus=%s\n", m_bCursor?"TRUE":"FALSE", m_bForcus?"TRUE":"FALSE" );
	...
	// ÉLÅ[É`ÉFÉbÉNópÉ^ÉCÉ}Å[
		// ÉAÉNÉeÉBÉuÉEÉCÉìÉhÉEÇ≈Ç»Ç¢éûÇ‚É_ÉCÉAÉçÉOìôÇ™èoÇƒÇ¢ÇÈéûÇÕÉLÉÉÉìÉZÉã
...
void	CMainFrame::OnEmulationStart( LPCSTR szFile, BOOL bChecked )
{
	...
	try {
		...
		if( (Nes = new NES(szFile)) ) 
		{
			...
			// ÉGÉ~ÉÖÉåÅ[ÉVÉáÉìÉXÉåÉbÉhÉXÉ^Å[Ég
...
//	DEBUGOUT( "CMainFrame::OnGraphicsCfg\n" );
	...
	if( dlg.DoModal( m_hWnd ) == IDOK ) {
		...
		// ÉTÅ[ÉtÉFÉXÇÉäÉrÉãÉhÇ∑ÇÈïKóvÇ™Ç†ÇÈÇ©ÇÃÉ`ÉFÉbÉN
		...
		if( Config.general.bScreenMode ) {
			// ÉXÉNÉäÅ[ÉìÉÇÅ[ÉhÇÃïœçXÇ™Ç†Ç¡ÇΩèÍçá
			if( DirectDraw.m_dwDisplayWidth  != Config.graphics.dwDisplayWidth
			 ...
			 || DirectDraw.m_dwDisplayRate   != Config.graphics.dwDisplayRate ) {
				...
				// ÉTÅ[ÉtÉFÉXÇÕçƒç\ízÇ≥ÇÍÇΩÇÃÇ≈ìÒìxÇÕçsÇÌÇ»Ç¢
...
void	CMainFrame::OnSetWindowSize()
{
	...
	// ÉÅÉjÉÖÅ[Ç™ê‹ÇËï‘Ç≥ÇÍÇÈÇ∆ècÇ™è¨Ç≥Ç≠Ç»ÇÈéñÇ™Ç†ÇÈÇÃÇ≈çƒèCê≥
	::GetClientRect( m_hWnd, &rcC );
	...
	// ç≈ëÂï\é¶éûÇ…ÉTÉCÉYÇïœçXÇµÇΩèÍçáÇÃç≈ëÂï\é¶ÇÃâèúãyÇ—É{É^ÉìÇÃçƒï`âÊ
...
void	CMainFrame::OnUpdateMenu( HMENU hMenu, UINT uID )
{
	...
	switch( uID ) {
		...
		case	ID_STATE_SLOT8:	case	ID_STATE_SLOT9:
			...
			CHAR	szMenuString[256];
			CHAR*	pToken;
			const UCHAR seps[] = " \t\0";	// ÉZÉpÉåÅ[É^

			// IDî‘çÜÇ©ÇÁÉCÉìÉfÉbÉNÉXÇíTÇ∑
...
BYTE	KeyBuf[256+64*8];
	...
	// É^ÉCÉ}Å[ÉCÉxÉìÉgÇ™èdÇ»ÇÈéñÇ™Ç†ÇÈÇÃÇ©Ç«Ç§Ç©ÇÌÇ©ÇÁÇÒÇ™Ç∆ÇËÇ†Ç¶Ç∏
	if( m_bKeyChecking )
		return;
	...
	// ÉLÅ[ÇÃéÊìæ
	if( !Emu.IsRunning() ) {
		// ÉGÉ~ÉÖÉåÅ[ÉVÉáÉìíÜà»äOÇÕÉ|Å[ÉäÉìÉOÇ∑ÇÈ
		DirectInput.Poll();
	}
	...
	BOOL	bAltOnly = FALSE;
	if( Emu.IsRunning() ) {
		// ÉtÉ@É~ÉäÅ[ÉxÅ[ÉVÉbÉNÉLÅ[É{Å[ÉhÇÃéû
